...
export class HostedContentsResponse implements Parsable {
    ...
    public getFieldDeserializers<T>() : Map<string, (item: T, node: ParseNode) => void> {
        return new Map<string, (item: T, node: ParseNode) => void>([
            ["@odata.nextLink", (o, n) => { (o as unknown as HostedContentsResponse).nextLink = n.getStringValue(); }],
            ["value", (o, n) => { (o as unknown as HostedContentsResponse).value = n.getCollectionOfObjectValues<ChatMessageHostedContent>(ChatMessageHostedContent); }],
        ]);
    };
    /**
