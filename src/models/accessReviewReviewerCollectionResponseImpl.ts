...
export class AccessReviewReviewerCollectionResponseImpl implements AccessReviewReviewerCollectionResponse {
    ...
    public constructor(accessReviewReviewerCollectionResponseParameterValue?: AccessReviewReviewerCollectionResponse | undefined) {
        this._additionalData = accessReviewReviewerCollectionResponseParameterValue?.additionalData ? accessReviewReviewerCollectionResponseParameterValue?.additionalData! : {};
        this._nextLink = accessReviewReviewerCollectionResponseParameterValue?.nextLink;
        this._value = accessReviewReviewerCollectionResponseParameterValue?.value;
    };
    /**
    ...
    public serialize(writer: SerializationWriter) : void {
        ...
        if(this.value && this.value.length != 0){        const valueArrValue: AccessReviewReviewerImpl[] = [];
        this.value?.forEach(element => {
            valueArrValue.push((element instanceof AccessReviewReviewerImpl? element as AccessReviewReviewerImpl:new AccessReviewReviewerImpl(element)));
        });
    ...
    public set value(value: AccessReviewReviewer[] | undefined) {
        if(value) {
            const valueArrValue: AccessReviewReviewerImpl[] = [];
            this.value?.forEach(element => {
                valueArrValue.push((element instanceof AccessReviewReviewerImpl? element as AccessReviewReviewerImpl:new AccessReviewReviewerImpl(element)));
            });
