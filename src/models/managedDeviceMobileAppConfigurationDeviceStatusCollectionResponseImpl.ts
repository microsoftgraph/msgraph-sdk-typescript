...
export class ManagedDeviceMobileAppConfigurationDeviceStatusCollectionResponseImpl implements AdditionalDataHolder, ManagedDeviceMobileAppConfigurationDeviceStatusCollectionResponse, Parsable {
    ...
    public serialize(writer: SerializationWriter) : void {
        ...
        if(this.value){
        const valueArrValue: ManagedDeviceMobileAppConfigurationDeviceStatusImpl[] = []; this.value?.forEach(element => {valueArrValue.push(new ManagedDeviceMobileAppConfigurationDeviceStatusImpl(element));});
