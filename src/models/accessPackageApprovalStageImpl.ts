...
export class AccessPackageApprovalStageImpl implements AccessPackageApprovalStage, AdditionalDataHolder, Parsable {
    ...
    public serialize(writer: SerializationWriter) : void {
        ...
        if(this.escalationApprovers){
        const escalationApproversArrValue: SubjectSetImpl[] = []; this.escalationApprovers?.forEach(element => {escalationApproversArrValue.push(new SubjectSetImpl(element));});
        writer.writeCollectionOfObjectValues<SubjectSetImpl>("escalationApprovers", escalationApproversArrValue);
        }
        if(this.fallbackEscalationApprovers){
        const fallbackEscalationApproversArrValue: SubjectSetImpl[] = []; this.fallbackEscalationApprovers?.forEach(element => {fallbackEscalationApproversArrValue.push(new SubjectSetImpl(element));});
        writer.writeCollectionOfObjectValues<SubjectSetImpl>("fallbackEscalationApprovers", fallbackEscalationApproversArrValue);
        }
        if(this.fallbackPrimaryApprovers){
        const fallbackPrimaryApproversArrValue: SubjectSetImpl[] = []; this.fallbackPrimaryApprovers?.forEach(element => {fallbackPrimaryApproversArrValue.push(new SubjectSetImpl(element));});
        ...
        if(this.primaryApprovers){
        const primaryApproversArrValue: SubjectSetImpl[] = []; this.primaryApprovers?.forEach(element => {primaryApproversArrValue.push(new SubjectSetImpl(element));});
