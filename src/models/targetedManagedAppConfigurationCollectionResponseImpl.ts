...
export class TargetedManagedAppConfigurationCollectionResponseImpl implements TargetedManagedAppConfigurationCollectionResponse {
    ...
    public constructor(targetedManagedAppConfigurationCollectionResponseParameterValue?: TargetedManagedAppConfigurationCollectionResponse | undefined) {
        this._additionalData = targetedManagedAppConfigurationCollectionResponseParameterValue?.additionalData ? targetedManagedAppConfigurationCollectionResponseParameterValue?.additionalData! : {};
        this._nextLink = targetedManagedAppConfigurationCollectionResponseParameterValue?.nextLink;
        this._value = targetedManagedAppConfigurationCollectionResponseParameterValue?.value;
    };
    /**
    ...
    public serialize(writer: SerializationWriter) : void {
        ...
        if(this.value && this.value.length != 0){        const valueArrValue: TargetedManagedAppConfigurationImpl[] = [];
        this.value?.forEach(element => {
            valueArrValue.push((element instanceof TargetedManagedAppConfigurationImpl? element as TargetedManagedAppConfigurationImpl:new TargetedManagedAppConfigurationImpl(element)));
        });
    ...
    public set value(value: TargetedManagedAppConfiguration[] | undefined) {
        if(value) {
            const valueArrValue: TargetedManagedAppConfigurationImpl[] = [];
            this.value?.forEach(element => {
                valueArrValue.push((element instanceof TargetedManagedAppConfigurationImpl? element as TargetedManagedAppConfigurationImpl:new TargetedManagedAppConfigurationImpl(element)));
            });
