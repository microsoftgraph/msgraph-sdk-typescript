...
export class GroupImpl extends DirectoryObjectImpl implements Group, Parsable {
    ...
    public serialize(writer: SerializationWriter) : void {
        ...
        if(this.acceptedSenders){
        const acceptedSendersArrValue: DirectoryObjectImpl[] = []; this.acceptedSenders?.forEach(element => {acceptedSendersArrValue.push(new DirectoryObjectImpl(element));});
        ...
        const assignedLabelsArrValue: AssignedLabelImpl[] = []; this.assignedLabels?.forEach(element => {assignedLabelsArrValue.push(new AssignedLabelImpl(element));});
        ...
        const conversationsArrValue: ConversationImpl[] = []; this.conversations?.forEach(element => {conversationsArrValue.push(new ConversationImpl(element));});
        ...
        if(this.groupLifecyclePolicies){
        const groupLifecyclePoliciesArrValue: GroupLifecyclePolicyImpl[] = []; this.groupLifecyclePolicies?.forEach(element => {groupLifecyclePoliciesArrValue.push(new GroupLifecyclePolicyImpl(element));});
        ...
        if(this.membersWithLicenseErrors){
        const membersWithLicenseErrorsArrValue: DirectoryObjectImpl[] = []; this.membersWithLicenseErrors?.forEach(element => {membersWithLicenseErrorsArrValue.push(new DirectoryObjectImpl(element));});
        ...
        const permissionGrantsArrValue: ResourceSpecificPermissionGrantImpl[] = []; this.permissionGrants?.forEach(element => {permissionGrantsArrValue.push(new ResourceSpecificPermissionGrantImpl(element));});
        ...
        if(this.rejectedSenders){
        const rejectedSendersArrValue: DirectoryObjectImpl[] = []; this.rejectedSenders?.forEach(element => {rejectedSendersArrValue.push(new DirectoryObjectImpl(element));});
        ...
        if(this.settings){
        const settingsArrValue: GroupSettingImpl[] = []; this.settings?.forEach(element => {settingsArrValue.push(new GroupSettingImpl(element));});
        ...
        if(this.transitiveMembers){
        const transitiveMembersArrValue: DirectoryObjectImpl[] = []; this.transitiveMembers?.forEach(element => {transitiveMembersArrValue.push(new DirectoryObjectImpl(element));});
