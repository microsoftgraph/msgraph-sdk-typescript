...
export class OptionalClaimsImpl implements OptionalClaims {
    ...
    public constructor(optionalClaimsParameterValue?: OptionalClaims | undefined) {
        const accessTokenArrValue: OptionalClaimImpl[] = []; this.accessToken?.forEach(element => {accessTokenArrValue.push(element instanceof OptionalClaimImpl? element : new OptionalClaimImpl(element));});
        ...
        const idTokenArrValue: OptionalClaimImpl[] = []; this.idToken?.forEach(element => {idTokenArrValue.push(element instanceof OptionalClaimImpl? element : new OptionalClaimImpl(element));});
        this.idToken = idTokenArrValue;
        const saml2TokenArrValue: OptionalClaimImpl[] = []; this.saml2Token?.forEach(element => {saml2TokenArrValue.push(element instanceof OptionalClaimImpl? element : new OptionalClaimImpl(element));});
