...
export class TimeOffCollectionResponseImpl implements TimeOffCollectionResponse {
    ...
    public constructor(timeOffCollectionResponseParameterValue?: TimeOffCollectionResponse | undefined) {
        this._additionalData = timeOffCollectionResponseParameterValue?.additionalData ? timeOffCollectionResponseParameterValue?.additionalData! : {};
        this._nextLink = timeOffCollectionResponseParameterValue?.nextLink;
        this._value = timeOffCollectionResponseParameterValue?.value;
    };
    /**
    ...
    public serialize(writer: SerializationWriter) : void {
        ...
        if(this.value && this.value.length != 0){        const valueArrValue: TimeOffImpl[] = [];
        this.value?.forEach(element => {
            valueArrValue.push((element instanceof TimeOffImpl? element as TimeOffImpl:new TimeOffImpl(element)));
        });
    ...
    public set value(value: TimeOff[] | undefined) {
        if(value) {
            const valueArrValue: TimeOffImpl[] = [];
            this.value?.forEach(element => {
                valueArrValue.push((element instanceof TimeOffImpl? element as TimeOffImpl:new TimeOffImpl(element)));
            });
