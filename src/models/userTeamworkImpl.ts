...
export class UserTeamworkImpl extends EntityImpl implements UserTeamwork {
    ...
    public constructor(userTeamworkParameterValue?: UserTeamwork | undefined) {
        ...
        this.associatedTeams = userTeamworkParameterValue?.associatedTeams;
    ...
    public serialize(writer: SerializationWriter) : void {
        ...
        if(this.associatedTeams && this.associatedTeams.length != 0){        const associatedTeamsArrValue: AssociatedTeamInfoImpl[] = []; this.associatedTeams?.forEach(element => {associatedTeamsArrValue.push(new AssociatedTeamInfoImpl(element));});
