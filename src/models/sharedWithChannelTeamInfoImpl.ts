...
export class SharedWithChannelTeamInfoImpl extends TeamInfoImpl implements SharedWithChannelTeamInfo {
    ...
    public constructor(sharedWithChannelTeamInfoParameterValue?: SharedWithChannelTeamInfo | undefined) {
        ...
        this.allowedMembers = sharedWithChannelTeamInfoParameterValue?.allowedMembers;
    ...
    public serialize(writer: SerializationWriter) : void {
        ...
        if(this.allowedMembers && this.allowedMembers.length != 0){        const allowedMembersArrValue: ConversationMemberImpl[] = []; this.allowedMembers?.forEach(element => {allowedMembersArrValue.push(new ConversationMemberImpl(element));});
