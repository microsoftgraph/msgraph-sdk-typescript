...
export class ChatMessageHostedContentCollectionResponseImpl implements ChatMessageHostedContentCollectionResponse {
    ...
    public constructor(chatMessageHostedContentCollectionResponseParameterValue?: ChatMessageHostedContentCollectionResponse | undefined) {
        this._additionalData = chatMessageHostedContentCollectionResponseParameterValue?.additionalData ? chatMessageHostedContentCollectionResponseParameterValue?.additionalData! : {};
        this._nextLink = chatMessageHostedContentCollectionResponseParameterValue?.nextLink;
        this._value = chatMessageHostedContentCollectionResponseParameterValue?.value;
    };
    /**
    ...
    public serialize(writer: SerializationWriter) : void {
        ...
        if(this.value && this.value.length != 0){        const valueArrValue: ChatMessageHostedContentImpl[] = [];
        this.value?.forEach(element => {
            valueArrValue.push((element instanceof ChatMessageHostedContentImpl? element as ChatMessageHostedContentImpl:new ChatMessageHostedContentImpl(element)));
        });
    ...
    public set value(value: ChatMessageHostedContent[] | undefined) {
        if(value) {
            const valueArrValue: ChatMessageHostedContentImpl[] = [];
            this.value?.forEach(element => {
                valueArrValue.push((element instanceof ChatMessageHostedContentImpl? element as ChatMessageHostedContentImpl:new ChatMessageHostedContentImpl(element)));
            });
