...
export class DeviceComplianceSettingStateCollectionResponseImpl implements DeviceComplianceSettingStateCollectionResponse {
    ...
    public constructor(deviceComplianceSettingStateCollectionResponseParameterValue?: DeviceComplianceSettingStateCollectionResponse | undefined) {
        this._additionalData = deviceComplianceSettingStateCollectionResponseParameterValue?.additionalData ? deviceComplianceSettingStateCollectionResponseParameterValue?.additionalData! : {};
        this._nextLink = deviceComplianceSettingStateCollectionResponseParameterValue?.nextLink;
        this._value = deviceComplianceSettingStateCollectionResponseParameterValue?.value;
    };
    /**
    ...
    public serialize(writer: SerializationWriter) : void {
        ...
        if(this.value && this.value.length != 0){        const valueArrValue: DeviceComplianceSettingStateImpl[] = [];
        this.value?.forEach(element => {
            valueArrValue.push((element instanceof DeviceComplianceSettingStateImpl? element as DeviceComplianceSettingStateImpl:new DeviceComplianceSettingStateImpl(element)));
        });
     ...
     */
    public set value(value: DeviceComplianceSettingState[] | undefined) {
        if(value) {
            const valueArrValue: DeviceComplianceSettingStateImpl[] = [];
            this.value?.forEach(element => {
                valueArrValue.push((element instanceof DeviceComplianceSettingStateImpl? element as DeviceComplianceSettingStateImpl:new DeviceComplianceSettingStateImpl(element)));
            });
