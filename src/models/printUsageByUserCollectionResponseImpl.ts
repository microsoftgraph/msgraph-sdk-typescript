...
export class PrintUsageByUserCollectionResponseImpl implements PrintUsageByUserCollectionResponse {
    ...
    public constructor(printUsageByUserCollectionResponseParameterValue?: PrintUsageByUserCollectionResponse | undefined) {
        this._additionalData = printUsageByUserCollectionResponseParameterValue?.additionalData ? printUsageByUserCollectionResponseParameterValue?.additionalData! : {};
        this._nextLink = printUsageByUserCollectionResponseParameterValue?.nextLink;
        this._value = printUsageByUserCollectionResponseParameterValue?.value;
    };
    /**
    ...
    public serialize(writer: SerializationWriter) : void {
        ...
        if(this.value && this.value.length != 0){        const valueArrValue: PrintUsageByUserImpl[] = [];
        this.value?.forEach(element => {
            valueArrValue.push((element instanceof PrintUsageByUserImpl? element as PrintUsageByUserImpl:new PrintUsageByUserImpl(element)));
        });
    ...
    public set value(value: PrintUsageByUser[] | undefined) {
        if(value) {
            const valueArrValue: PrintUsageByUserImpl[] = [];
            this.value?.forEach(element => {
                valueArrValue.push((element instanceof PrintUsageByUserImpl? element as PrintUsageByUserImpl:new PrintUsageByUserImpl(element)));
            });
