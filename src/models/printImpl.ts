...
export class PrintImpl implements AdditionalDataHolder, Parsable, Print {
    ...
    public serialize(writer: SerializationWriter) : void {
        ...
        if(this.connectors){
        const connectorsArrValue: PrintConnectorImpl[] = []; this.connectors?.forEach(element => {connectorsArrValue.push(new PrintConnectorImpl(element));});
        writer.writeCollectionOfObjectValues<PrintConnectorImpl>("connectors", connectorsArrValue);
        }
        if(this.operations){
        const operationsArrValue: PrintOperationImpl[] = []; this.operations?.forEach(element => {operationsArrValue.push(new PrintOperationImpl(element));});
        writer.writeCollectionOfObjectValues<PrintOperationImpl>("operations", operationsArrValue);
        }
        if(this.printers){
        const printersArrValue: PrinterImpl[] = []; this.printers?.forEach(element => {printersArrValue.push(new PrinterImpl(element));});
        writer.writeCollectionOfObjectValues<PrinterImpl>("printers", printersArrValue);
        }
        if(this.services){
        const servicesArrValue: PrintServiceImpl[] = []; this.services?.forEach(element => {servicesArrValue.push(new PrintServiceImpl(element));});
        ...
        if(this.shares){
        const sharesArrValue: PrinterShareImpl[] = []; this.shares?.forEach(element => {sharesArrValue.push(new PrinterShareImpl(element));});
        writer.writeCollectionOfObjectValues<PrinterShareImpl>("shares", sharesArrValue);
        }
        if(this.taskDefinitions){
        const taskDefinitionsArrValue: PrintTaskDefinitionImpl[] = []; this.taskDefinitions?.forEach(element => {taskDefinitionsArrValue.push(new PrintTaskDefinitionImpl(element));});
