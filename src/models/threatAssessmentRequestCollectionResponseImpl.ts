...
export class ThreatAssessmentRequestCollectionResponseImpl implements ThreatAssessmentRequestCollectionResponse {
    ...
    public constructor(threatAssessmentRequestCollectionResponseParameterValue?: ThreatAssessmentRequestCollectionResponse | undefined) {
        this._additionalData = threatAssessmentRequestCollectionResponseParameterValue?.additionalData ? threatAssessmentRequestCollectionResponseParameterValue?.additionalData! : {};
        this._nextLink = threatAssessmentRequestCollectionResponseParameterValue?.nextLink;
        this._value = threatAssessmentRequestCollectionResponseParameterValue?.value;
    };
    /**
    ...
    public serialize(writer: SerializationWriter) : void {
        ...
        if(this.value && this.value.length != 0){        const valueArrValue: ThreatAssessmentRequestImpl[] = [];
        this.value?.forEach(element => {
            valueArrValue.push((element instanceof ThreatAssessmentRequestImpl? element as ThreatAssessmentRequestImpl:new ThreatAssessmentRequestImpl(element)));
        });
    ...
    public set value(value: ThreatAssessmentRequest[] | undefined) {
        if(value) {
            const valueArrValue: ThreatAssessmentRequestImpl[] = [];
            this.value?.forEach(element => {
                valueArrValue.push((element instanceof ThreatAssessmentRequestImpl? element as ThreatAssessmentRequestImpl:new ThreatAssessmentRequestImpl(element)));
            });
