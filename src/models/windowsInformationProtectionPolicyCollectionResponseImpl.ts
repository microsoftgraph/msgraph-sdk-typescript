...
export class WindowsInformationProtectionPolicyCollectionResponseImpl implements WindowsInformationProtectionPolicyCollectionResponse {
    ...
    public constructor(windowsInformationProtectionPolicyCollectionResponseParameterValue?: WindowsInformationProtectionPolicyCollectionResponse | undefined) {
        this._additionalData = windowsInformationProtectionPolicyCollectionResponseParameterValue?.additionalData ? windowsInformationProtectionPolicyCollectionResponseParameterValue?.additionalData! : {};
        this._nextLink = windowsInformationProtectionPolicyCollectionResponseParameterValue?.nextLink;
        this._value = windowsInformationProtectionPolicyCollectionResponseParameterValue?.value;
    };
    /**
    ...
    public serialize(writer: SerializationWriter) : void {
        ...
        if(this.value && this.value.length != 0){        const valueArrValue: WindowsInformationProtectionPolicyImpl[] = [];
        this.value?.forEach(element => {
            valueArrValue.push((element instanceof WindowsInformationProtectionPolicyImpl? element as WindowsInformationProtectionPolicyImpl:new WindowsInformationProtectionPolicyImpl(element)));
        });
    ...
    public set value(value: WindowsInformationProtectionPolicy[] | undefined) {
        if(value) {
            const valueArrValue: WindowsInformationProtectionPolicyImpl[] = [];
            this.value?.forEach(element => {
                valueArrValue.push((element instanceof WindowsInformationProtectionPolicyImpl? element as WindowsInformationProtectionPolicyImpl:new WindowsInformationProtectionPolicyImpl(element)));
            });
