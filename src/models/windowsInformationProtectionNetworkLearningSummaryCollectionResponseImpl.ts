...
export class WindowsInformationProtectionNetworkLearningSummaryCollectionResponseImpl implements AdditionalDataHolder, Parsable, WindowsInformationProtectionNetworkLearningSummaryCollectionResponse {
    ...
    public serialize(writer: SerializationWriter) : void {
        ...
        if(this.value){
        const valueArrValue: WindowsInformationProtectionNetworkLearningSummaryImpl[] = []; this.value?.forEach(element => {valueArrValue.push(new WindowsInformationProtectionNetworkLearningSummaryImpl(element));});
