...
export class AccessReviewInstanceImpl extends EntityImpl implements AccessReviewInstance, Parsable {
    ...
    public serialize(writer: SerializationWriter) : void {
        ...
        if(this.contactedReviewers){
        const contactedReviewersArrValue: AccessReviewReviewerImpl[] = []; this.contactedReviewers?.forEach(element => {contactedReviewersArrValue.push(new AccessReviewReviewerImpl(element));});
        writer.writeCollectionOfObjectValues<AccessReviewReviewerImpl>("contactedReviewers", contactedReviewersArrValue);
        }
        if(this.decisions){
        const decisionsArrValue: AccessReviewInstanceDecisionItemImpl[] = []; this.decisions?.forEach(element => {decisionsArrValue.push(new AccessReviewInstanceDecisionItemImpl(element));});
