...
export class TeamImpl extends EntityImpl implements Parsable, Team {
    ...
    public serialize(writer: SerializationWriter) : void {
        ...
        if(this.channels){
        const channelsArrValue: ChannelImpl[] = []; this.channels?.forEach(element => {channelsArrValue.push(new ChannelImpl(element));});
        ...
        if(this.operations){
        const operationsArrValue: TeamsAsyncOperationImpl[] = []; this.operations?.forEach(element => {operationsArrValue.push(new TeamsAsyncOperationImpl(element));});
