...
export class ConnectedOrganizationImpl extends EntityImpl implements ConnectedOrganization, Parsable {
    ...
    public serialize(writer: SerializationWriter) : void {
        ...
        if(this.externalSponsors){
        const externalSponsorsArrValue: DirectoryObjectImpl[] = []; this.externalSponsors?.forEach(element => {externalSponsorsArrValue.push(new DirectoryObjectImpl(element));});
        writer.writeCollectionOfObjectValues<DirectoryObjectImpl>("externalSponsors", externalSponsorsArrValue);
        }
        if(this.identitySources){
        const identitySourcesArrValue: IdentitySourceImpl[] = []; this.identitySources?.forEach(element => {identitySourcesArrValue.push(new IdentitySourceImpl(element));});
        writer.writeCollectionOfObjectValues<IdentitySourceImpl>("identitySources", identitySourcesArrValue);
        }
        if(this.internalSponsors){
        const internalSponsorsArrValue: DirectoryObjectImpl[] = []; this.internalSponsors?.forEach(element => {internalSponsorsArrValue.push(new DirectoryObjectImpl(element));});
