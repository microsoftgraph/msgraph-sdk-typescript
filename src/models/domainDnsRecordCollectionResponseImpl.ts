...
export class DomainDnsRecordCollectionResponseImpl implements DomainDnsRecordCollectionResponse {
    ...
    public constructor(domainDnsRecordCollectionResponseParameterValue?: DomainDnsRecordCollectionResponse | undefined) {
        this._additionalData = domainDnsRecordCollectionResponseParameterValue?.additionalData ? domainDnsRecordCollectionResponseParameterValue?.additionalData! : {};
        this._nextLink = domainDnsRecordCollectionResponseParameterValue?.nextLink;
        this._value = domainDnsRecordCollectionResponseParameterValue?.value;
    };
    /**
    ...
    public getFieldDeserializers() : Record<string, (node: ParseNode) => void> {
        return {
            ...
            "value": n => { this.value = n.getCollectionOfObjectValues<DomainDnsRecordImpl>(createDomainDnsRecordFromDiscriminatorValue); },
        };
    };
    /**
    ...
    public serialize(writer: SerializationWriter) : void {
        ...
        if(this.value && this.value.length != 0){        const valueArrValue: DomainDnsRecordImpl[] = [];
        this.value?.forEach(element => {
            valueArrValue.push((element instanceof DomainDnsRecordImpl? element as DomainDnsRecordImpl:new DomainDnsRecordImpl(element)));
        });
    ...
    public set value(value: DomainDnsRecord[] | undefined) {
        if(value) {
            const valueArrValue: DomainDnsRecordImpl[] = [];
            this.value?.forEach(element => {
                valueArrValue.push((element instanceof DomainDnsRecordImpl? element as DomainDnsRecordImpl:new DomainDnsRecordImpl(element)));
            });
