...
export class DetectedAppCollectionResponseImpl implements DetectedAppCollectionResponse {
    ...
    public constructor(detectedAppCollectionResponseParameterValue?: DetectedAppCollectionResponse | undefined) {
        this._additionalData = detectedAppCollectionResponseParameterValue?.additionalData ? detectedAppCollectionResponseParameterValue?.additionalData! : {};
        this._nextLink = detectedAppCollectionResponseParameterValue?.nextLink;
        this._value = detectedAppCollectionResponseParameterValue?.value;
    };
    /**
    ...
    public serialize(writer: SerializationWriter) : void {
        ...
        if(this.value && this.value.length != 0){        const valueArrValue: DetectedAppImpl[] = [];
        this.value?.forEach(element => {
            valueArrValue.push((element instanceof DetectedAppImpl? element as DetectedAppImpl:new DetectedAppImpl(element)));
        });
    ...
    public set value(value: DetectedApp[] | undefined) {
        if(value) {
            const valueArrValue: DetectedAppImpl[] = [];
            this.value?.forEach(element => {
                valueArrValue.push((element instanceof DetectedAppImpl? element as DetectedAppImpl:new DetectedAppImpl(element)));
            });
