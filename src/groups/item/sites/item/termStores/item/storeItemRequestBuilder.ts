...
export class StoreItemRequestBuilder {
    ...
    /**
     * Gets an item from the MicrosoftGraph.groups.item.sites.item.termStores.item.groups.item collection
     ...
     * @param responseHandler Response handler to use in place of the default response handling provided by the core service
     */
    public patch(body: StoreImpl | undefined, requestConfiguration?: StoreItemRequestBuilderPatchRequestConfiguration | undefined, responseHandler?: ResponseHandler | undefined) : Promise<void> {
        if(!body) throw new Error("body cannot be undefined");
    ...
    /**
     * Gets an item from the MicrosoftGraph.groups.item.sites.item.termStores.item.sets.item collection
     * @param id Unique identifier of the item
