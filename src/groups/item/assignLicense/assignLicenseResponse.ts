...
export class AssignLicenseResponse implements Parsable {
    ...
    public getFieldDeserializers<T>() : Map<string, (item: T, node: ParseNode) => void> {
        return new Map<string, (item: T, node: ParseNode) => void>([
            ["addLicenses", (o, n) => { (o as unknown as AssignLicenseResponse).addLicenses = n.getCollectionOfObjectValues<AssignedLicense>(AssignedLicense); }],
            ["removeLicenses", (o, n) => { (o as unknown as AssignLicenseResponse).removeLicenses = n.getCollectionOfPrimitiveValues<string>(); }],
        ]);
    };
    /**
