...
export class TaskDefinitionRequestBuilder extends BaseRequestBuilder {
     ...
     */
    public get(requestConfiguration?: TaskDefinitionRequestBuilderGetRequestConfiguration | undefined, responseHandler?: ResponseHandler | undefined) : Promise<TaskDefinition | undefined> {
        ...
        return this.requestAdapter?.sendAsync<TaskDefinition>(requestInfo, createTaskDefinitionFromDiscriminatorValue, responseHandler, errorMapping) ?? Promise.reject(new Error('request adapter is null'));
    };
    /**
     * The taskDefinition associated with the related lifecycle workflow task.Supports $filter(eq, ne) and $expand.
     * @param requestConfiguration Configuration for the request such as headers, query parameters, and middleware options.
