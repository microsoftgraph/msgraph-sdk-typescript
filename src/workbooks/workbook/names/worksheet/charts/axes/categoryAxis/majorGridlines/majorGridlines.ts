...
export class MajorGridlines extends Entity implements Parsable {
    ...
    public getFieldDeserializers<T>() : Map<string, (item: T, node: ParseNode) => void> {
        return new Map<string, (item: T, node: ParseNode) => void>([...super.getFieldDeserializers<T>(),
            ["format", (o, n) => { (o as unknown as MajorGridlines).format = n.getObjectValue<WorkbookChartGridlinesFormat>(WorkbookChartGridlinesFormat); }],
            ["visible", (o, n) => { (o as unknown as MajorGridlines).visible = n.getBooleanValue(); }],
        ]);
    };
    /**
