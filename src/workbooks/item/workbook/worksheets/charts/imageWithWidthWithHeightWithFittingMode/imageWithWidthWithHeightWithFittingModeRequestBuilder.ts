...
export class ImageWithWidthWithHeightWithFittingModeRequestBuilder {
     ...
     */
    public constructor(currentPath: string, httpCore: HttpCore, width?: number | undefined, height?: number | undefined, fittingMode?: string | undefined, isRawUrl: boolean = true) {
        if(!currentPath) throw new Error("currentPath cannot be undefined");
        if(!httpCore) throw new Error("httpCore cannot be undefined");
        this.pathSegment = `/microsoft.graph.image(width=${width ?? ''},height=${height ?? ''},fittingMode='${fittingMode ?? ''}')`;
