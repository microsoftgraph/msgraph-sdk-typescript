"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.Microsoft = void 0;
var kiota_abstractions_1 = require("@microsoft/kiota-abstractions");
var Microsoft = /** @class */ (function () {
    function Microsoft() {
    }
    return Microsoft;
}());
exports.Microsoft = Microsoft;
graph.substituteResponse;
implements;
kiota_abstractions_1.Parsable;
{
    _additionalData: Map();
    _instanceNum ?  : Json | undefined;
    _newText ?  : Json | undefined;
    _oldText ?  : Json | undefined;
    _text ?  : Json | undefined;
    constructor();
    {
        this._additionalData = new Map();
    }
    ;
    get;
    additionalData();
    {
        return this._additionalData;
    }
    ;
    get;
    instanceNum();
    {
        return this._instanceNum;
    }
    ;
    get;
    newText();
    {
        return this._newText;
    }
    ;
    get;
    oldText();
    {
        return this._oldText;
    }
    ;
    get;
    text();
    {
        return this._text;
    }
    ;
    getFieldDeserializers();
    Map < string, function (item, node) { return void  > {
        return: new Map([
            ["instanceNum", function (o, n) { o.instanceNum = n.getObjectValue(Json); }],
            ["newText", function (o, n) { o.newText = n.getObjectValue(Json); }],
            ["oldText", function (o, n) { o.oldText = n.getObjectValue(Json); }],
            ["text", function (o, n) { o.text = n.getObjectValue(Json); }],
        ])
    }; };
    serialize(writer, kiota_abstractions_1.SerializationWriter);
    void {
        if: function (, writer) { },
        throw: new Error("writer cannot be undefined"),
        writer: writer,
        : .writeObjectValue("instanceNum", this.instanceNum),
        writer: writer,
        : .writeObjectValue("newText", this.newText),
        writer: writer,
        : .writeObjectValue("oldText", this.oldText),
        writer: writer,
        : .writeObjectValue("text", this.text),
        writer: writer,
        : .writeAdditionalData(this.additionalData)
    };
    set;
    additionalData(value, Map(), {
        this: ._additionalData = value
    });
    set;
    instanceNum(value, Json | undefined);
    {
        this._instanceNum = value;
    }
    ;
    set;
    newText(value, Json | undefined);
    {
        this._newText = value;
    }
    ;
    set;
    oldText(value, Json | undefined);
    {
        this._oldText = value;
    }
    ;
    set;
    text(value, Json | undefined);
    {
        this._text = value;
    }
    ;
}
